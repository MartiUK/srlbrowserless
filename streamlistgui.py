...
#get a list of available qualities for a given streamer, or raise if the streamer is offline
...
#draw some stuff
stdscr.border(0)
stdscr.addstr(0, (curses.COLS/2)-24, "SpeedRunsLive Unofficial Command-Line GUI Alpha")
stdscr.refresh()
...
listwin = curses.newwin(yoffset+padh, xoffset+padw-1, yoffset-1, xoffset-1)
redraw(listwin, "Loading stream list...", padh, padw) #need win-1, so use pad dims cos they're win-1
  ...
  if streamselection:
    if cmd == curses.KEY_DOWN:
      ...
      try:
        ...
        #don't need to delete streamlist here since we're just reinitialising
        redraw(listwin)
        streams, streamlist = loadStreams(yoffset, xoffset, padh, padw)
      else:
        #could probably get away with not deleting it here but w/e it's memory shit
        del streamlist
        redraw(listwin)
        qualitylist = scrolllist.ScrollList(yoffset, xoffset, padh, padw, [x["label"] for x in qualities])
        qualitylist.refresh()
